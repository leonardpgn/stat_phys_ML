...
class Model:
    ...
    def train(self, data_t, data_y, learning_rate=0.001, epochs=100, show_status=True):
    ...
    def loocv(self, data_t, data_y, show_info=True):
        losses = []
        for i, (ti, yi) in enumerate(zip(data_t, data_y)):
            ...
            self.reset_weights()
            self.train(train_t, train_y, show_status=False)
